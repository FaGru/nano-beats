export const convertBufferToWav = (
  sampleRate: any,
  channelBuffers: any,
  returnType: 'blob' | 'audioUrl' | 'buffer' = 'buffer'
) => {
  if (!Array.isArray(channelBuffers)) {
    channelBuffers = [channelBuffers];
  }
  const totalSamples = channelBuffers[0].length * channelBuffers.length || channelBuffers.length;
  ...
  L: if (returnType === 'blob') {
    const blob = new Blob([buffer], { type: 'audio/wav' });
    return blob;
  }
  if (returnType === 'audioUrl') {
    const blob = new Blob([buffer], { type: 'audio/wav' });
